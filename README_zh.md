...
所有请求均通过 **HMAC-SHA256** 签名进行身份验证。你需要在 [Neburst 控制面板](https://dash.neburst.com) 中创建 API Key（包含 `key_id` 和 `secret`）。
...
完整的签名算法和示例请参阅 [docs/authentication.md](docs/authentication.md)。
...
完整的请求与响应文档请参阅 [docs/endpoints.md](docs/endpoints.md)。

### 速率限制

每用户每分钟 60 次请求。详见 [docs/rate-limiting.md](docs/rate-limiting.md)。
...
client = NeburstClient("https://api.neburst.com", "nb_key_...", "nb_secret_...")
...
var client = new NeburstClient("https://api.neburst.com", "nb_key_...", "nb_secret_...");
...
let client = NeburstClient::new("https://api.neburst.com", "nb_key_...", "nb_secret_...");
let instances = client.list_instances().await?;
```

### 文档

- [认证方式](docs/authentication.md) - HMAC-SHA256 签名流程
- [接口文档](docs/endpoints.md) - 完整 API 参考
- [权限范围](docs/scopes.md) - 权限系统说明
- [速率限制](docs/rate-limiting.md) - 请求频率限制
- [错误码](docs/errors.md) - 错误码参考
