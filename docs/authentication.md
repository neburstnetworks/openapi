...
When you create an API key in the [Neburst Dashboard](https://dash.neburst.com), you will receive a **combined key** -- a single Base64-encoded string that contains both the Key ID and Secret:
...
Decoding this Base64 string yields a JSON object:
...
All official SDKs accept the combined key directly -- you can pass it as a single argument instead of providing `key_id` and `secret` separately:

```go
// Combined key (single argument)
client := neburst.NewClient("https://api.neburst.com", "eyJrZXlfaWQ...")

// Equivalent to separate key_id + secret
client := neburst.NewClient("https://api.neburst.com", "nb_key_...", "nb_secret_...")
```

The CLI also accepts combined keys when adding an account:

```bash
neburst-cli account add my-account
# Paste the combined key when prompted for "API Key"
```
